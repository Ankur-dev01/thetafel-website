...
// clicking the emailed link is the confirmation.
...
type VerifyState = 'sending' | 'success' | 'failure'

export default function DataRequestVerifyPage() {
  const t = useTranslations('privacy.dataRequest.verify')
  ...
  useEffect(() => {
    ...
    async function verify() {
      try {
        const res = await fetch('/api/consumer/privacy/data-request/verify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ token }),
        })
        const data = await res.json().catch(() => ({}))
        setState(res.ok && data?.ok ? 'success' : 'failure')
      } catch {
        setState('failure')
      }
    }
...
function SendingIcon() {
  return (
    ...
    >
      ...
      />
      <path
        d="M14 3C19.5228 3 24 7.47715 24 13"
        stroke="#d4820a"
        strokeWidth="3"
        strokeLinecap="round"
      />
