// app/api/consumer/privacy/data-deletion/route.ts
//
// POST /api/consumer/privacy/data-deletion
//
...
import { dataDeletionInputSchema } from '@/lib/consumer/schemas/privacySchema'
...
import { sendDataDeletionLinkEmail } from '@/lib/consumer/notifications/dispatchDataDeletion'
...
export async function POST(req: NextRequest) {
  ...
  // 1. Rate limit — shares the data_request key with the export flow so a
  //    guest can't spam both endpoints in parallel.
  ...
  const parsed = dataDeletionInputSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json(
  ...
  try {
    ...
    if (guest) {
      ...
      if (created.ok) {
        ...
        const verifyUrl = `${base}${localePrefix}/privacybeleid/data-deletion/verify?token=${encodeURIComponent(created.token)}`

        void sendDataDeletionLinkEmail({
          locale: input.locale,
          guestFullName: guest.full_name,
          guestEmail: guest.email,
          verifyUrl,
        }).catch((err) => {
          console.error('[privacy/data-deletion] link email dispatch failed', err)
        })
      } else {
        console.error('[privacy/data-deletion] createMagicLink failed', created.reason)
      }
    }
  } catch (err) {
    console.error('[privacy/data-deletion] unexpected error', err)
  }
