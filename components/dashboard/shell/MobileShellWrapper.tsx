  ...
  return (
    <div className="min-h-screen bg-cream text-[#1e1508]">
      {/* Desktop sidebar — fixed to viewport; icon rail 768–1100px, full ≥ 1100px.
          Ranges are non-overlapping (max-[1099px] / min-[1100px]) on purpose —
          two competing breakpoint utilities on the same property (e.g. bare
          `w-16 min-[1100px]:w-60`) don't reliably cascade the wider one on
          top at ≥1100px, which silently broke click targeting on every
          dashboard page until this was caught in D1.3. */}
      <aside className="hidden md:flex md:flex-col fixed top-0 left-0 h-screen max-md:w-0 md:max-[1099px]:w-16 min-[1100px]:w-60 bg-[#1e1508] text-[#fdfaf5] overflow-y-auto z-20">
        {sidebar}
      </aside>

      {/* Main pane */}
      <div className="max-md:pl-0 md:max-[1099px]:pl-16 min-[1100px]:pl-60 min-h-screen flex flex-col">
        {header}
