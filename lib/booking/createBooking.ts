...
export async function createBooking(
  ...
  // ── 2. Pick a specific table (smallest-fits-first) ────────────────────────
  ...
  for (const zoneId of candidateZoneIds) {
    ...
    const fitting = zone.tables
      .filter((t) => t.seats >= input.partySize)
  ...
  // ── 8. Insert magic_links row (FK satisfied — booking exists) ─────────────
